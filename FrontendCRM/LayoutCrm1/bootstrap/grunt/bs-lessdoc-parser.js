...
let CUSTOMIZABLE_HEADING = /^[/]{2}={2}(.*)$/;
let UNCUSTOMIZABLE_HEADING = /^[/]{2}-{2}(.*)$/;
let SUBSECTION_HEADING = /^[/]{2}={3}(.*)$/;
let SECTION_DOCSTRING = /^[/]{2}#{2}(.+)$/;
let VAR_ASSIGNMENT = /^(@[a-zA-Z0-9_-]+):[ ]*([^ ;][^;]*);[ ]*$/;
let VAR_DOCSTRING = /^[/]{2}[*]{2}(.+)$/;
...
Tokenizer.prototype._shift = function () {
  ...
  let commentStart = line.lastIndexOf('//');
  let varLine = commentStart === -1 ? line : line.slice(0, commentStart);
...
Parser.prototype.parseSection = function () {
  let section = this._tokenizer.shift();
  if (section === null) {
    return null;
  }
...
Parser.prototype.parseSubSection = function () {
  let subsection = this._tokenizer.shift();
...
Parser.prototype.parseVar = function () {
  ...
  let variable = this._tokenizer.shift();
