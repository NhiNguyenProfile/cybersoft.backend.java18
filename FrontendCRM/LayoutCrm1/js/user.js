$(document).ready(function () {
  ...
  $.ajax({
    ...
    $.each(result, function (index, value) {
      ...
      let option = `<option role-id=${value.id}>${value.name}</option>`;
  ...
  $("#btn-save-user").click(function () {
    let fullname = document.getElementById("nameUser").value;
    let email = document.getElementById("example-email").value;
    let password = document.getElementById("passUser").value;
    let roleName = document.getElementById("roleOptions").value;
    let roleId = localStorage.getItem(`${roleName}`);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          data: { fullname, email, password, roleId },
        }).done((result) => {
  ...
  $("body").on("click", "#updateUser", function () {
    ...
    let userEmail = document.getElementById("upUserEmail").value;
    let userPassword = document.getElementById("upPassUser").value;
    let userFullname = document.getElementById("upFullname").value;
    let userRoleId = localStorage.getItem(
  ...
  $("body").on("click", ".btn-deleteUser", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `http://localhost:8081/cybersoft.backend.java18/api/users?id=${userId}`,
          method: "DELETE",
          dataType: "json",
        }).done((result) => {
          if (result.isSuccess == true) {
